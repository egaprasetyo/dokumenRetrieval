...
public class testTFIDF3 {
    ...
    public static void main(String[] args) {
        ...
        for (int i = 0; i < result.size(); i++) {
            System.out.println("Term = " + result.get(i).getTerm() + " , numberofDucument = " + result.get(i).getNumberofDocument());
            ArrayList<Posting> tempPosting = result.get(i).getPostingList();
            for (int j = 0; j < tempPosting.size(); j++) {
                System.out.println("Id_doc = " + tempPosting.get(j).getDocument().getId() + " , numberofTerm = " + tempPosting.get(j).getNumberOfTerm());
